...
class Utilities:
    def __init__(self, tokenizer, model, part):
    ...
    def sanity_check(self, sentence, block_size):
        ...
        input_tensor = torch.tensor(padded_sentence, dtype=torch.long).to(device).unsqueeze(0)
        ...
        for j, attn_map in enumerate(attn_maps):
            att_map = attn_map.mean(dim=1).squeeze(0).detach().cpu().numpy()  # Remove batch dimension and convert to NumPy array

            # Check if the attention probabilities sum to 1 over rows
            total_prob_over_rows = torch.sum(attn_map, dim=-1).detach().cpu()
            if torch.any(total_prob_over_rows < 0.99) or torch.any(total_prob_over_rows > 1.01):
                print(f"Failed normalization test for Attention Map {j + 1}: probabilities do not sum to 1.0 over rows")
            ...
            # Save the plot
            plt.savefig(f"attention_map_{j + 1}_part_{self.part}.png")
